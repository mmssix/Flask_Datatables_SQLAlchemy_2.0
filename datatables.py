"""
https://github.com/orf/datatables/
"""
...
# from app.utils.functions import error_logger, local_to_utc
...
class DataTable(object):
    ...
    def _list_of_dicts(self):
        ...
        columns = self.query_into_dict("columns")
        if not columns:
            columns = {k:v for k,v in enumerate(self.params['columns'])}
        ordering = self.query_into_dict("order")
        if not ordering:
            ordering = {k:v for k,v in enumerate(self.params['order'])}
        search = self.query_into_dict("search")
        if not search:
            search = {k:v for k,v in enumerate(self.params['search'])}
        ...
        return [self.output_instance(instance) for instance in query.all()]

    def list_of_dicts(self):
        """
        Will return all results from filtered resultset as a list of dicts
        :return:
        """
        try:
    ...
    # def return_report(self, filename="export", tz="America/New_York"):
    #     """
    #     Will return a response object with a csv
    #     :param tz:
    #     :return:
    #     """
    #     df = pd.DataFrame.from_records(self._list_of_dicts())
    #
    #     # localize date columns so that converting to excel is possible
    #     v = np.vectorize(local_to_utc)
